...
if geoip2:
    try:
        ...
        _logger.warning(f"⚠️ Failed to create empty GEOIP objects: {e}")
        class EmptyGeoIPObject:
            def __getattr__(self, attr):
                return None
        GEOIP_EMPTY_COUNTRY = EmptyGeoIPObject()
        GEOIP_EMPTY_CITY = EmptyGeoIPObject()
...
class GeoIP(collections.abc.Mapping):
    ...
    def __getattr__(self, attr):
        if hasattr(GEOIP_EMPTY_COUNTRY, attr):
            return getattr(self._country_record, attr, None)
        if hasattr(GEOIP_EMPTY_CITY, attr):
            return getattr(self._city_record, attr, None)
        return None  # <-- always return None if missing attribute
